...
const HeartHandshake = ({ className }) => (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z" /><path d="M12 5 9.04 7.96a2.17 2.17 0 0 0 0 3.08v0c.82.82 2.13.82 2.94 0l.06-.06L12 11l2.96-2.96a2.17 2.17 0 0 0 0-3.08v0c-.82-.82-2.13-.82-2.94 0l-.06.06Z" /></svg>);
const Scale = ({ className }) => (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="m16 16-4-4-4 4" /><path d="M12 2v12" /><path d="M4 12H2" /><path d="M10 12H8" /><path d="M16 12h-2" /><path d="M22 12h-2" /><path d="M6 6l-4 4-4-4" /><path d="M18 6l4 4-4 4" /></svg>);
const Network = ({ className }) => (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><rect x="16" y="16" width="6" height="6" rx="1"/><rect x="2" y="16" width="6" height="6" rx="1"/><rect x="9" y="2" width="6" height="6" rx="1"/><path d="M5 16v-3a1 1 0 0 1 1-1h12a1 1 0 0 1 1 1v3"/><path d="M12 12V8"/></svg>);
const Car = ({ className }) => (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M14 16.94V18a2 2 0 0 1-2 2h-4a2 2 0 0 1-2-2v-1.06"/><path d="m10 16-1.3-2.6A1 1 0 0 0 7.8 13H4a1 1 0 0 0-1 1v1"/><path d="M14 16h2a1 1 0 0 0 1-1v-1a1 1 0 0 0-.8-1l-2.2-.4"/><path d="M5 11h1"/><path d="M18 11h1"/><path d="m3 9 2.3 2.3"/><path d="m19 9-2.3 2.3"/><path d="m12 4 1.8 3.6"/><path d="m9 2-1.8 3.6"/><path d="M12 4V2"/><circle cx="12" cy="11" r="1"/></svg>);
const ShieldCheck = ({ className }) => (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="m9 12 2 2 4-4"/></svg>);
...
const Dna = ({ className }) => (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M4 14.5A3.5 3.5 0 0 1 7.5 11H12"/><path d="M16.5 11a3.5 3.5 0 1 1 0 7H12"/><path d="M12 11V4.5A3.5 3.5 0 0 1 8.5 8H4"/><path d="M12 18v-7"/><path d="M8.5 14.5a3.5 3.5 0 1 1 0-7H12"/></svg>);
const KeyRound = ({ className }) => (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M2 18v3c0 .6.4 1 1 1h4v-3h3v-3h2l1.4-1.4a6.5 6.5 0 1 0-4-4Z"/><circle cx="16.5" cy="7.5" r=".5"/></svg>);
...
export default function App() {
    ...
    const callGemini = async (systemPrompt, userPrompt, retries = 3, delay = 1000) => {
    ...
    const handleGenerateScenario = async () => {
        ...
        setPerspective('');
        setUserInput('');
        const systemPrompt = "You are a futurist and ethicist. Create a concise, thought-provoking  dilemma (under 60 words) related to symbiotic AI, cognitive augmentation, or data sovereignty in a future where systems like NEHA exist. The scenario should be complex and have no easy answer.";
    ...
    const handleGetPerspective = async () => {
        if (!userInput.trim()) {
            setError("Please provide your perspective before seeking NEHA's analysis.");
            return;
        }
            ...
            <main ref={ref} className="bg-transparent text-gray-200 overflow-x-hidden relative">
                ...
                <header className="fixed top-0 left-0 right-0 z-50 flex justify-between items-center p-6 bg-black/10 backdrop-blur-lg border-b border-gray-500/10">
                    ...
                    <nav className="hidden md:flex items-center space-x-8 text-sm text-gray-400">
                        <a href="#core" className="hover:text-white transition-colors duration-300">Triadic Core</a>
                        <a href="#ethics" className="hover:text-white transition-colors duration-300"> Framework</a>
                ...
                <section className="h-screen min-h-[700px] flex flex-col items-center justify-center text-center px-4 relative overflow-hidden">
                    ...
                    <div className="z-10">
                        ...
                        <motion.h1 variants={titleVariants} initial="hidden" animate="visible" className="text-5xl md:text-7xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-br from-white to-gray-400 max-w-4xl mx-auto">
...
Guiding human progress by combining powerful intelligence with deep wisdom and emotional intelligence.
...
<h2 className="text-4xl md:text-5xl font-bold tracking-tight">The Triadic Core</h2>

<p className="mt-4 text-gray-400 max-w-2xl mx-auto text-lg">NEHA is built on three core pillars that work together, creating a powerful foundation for guidance and understanding.</p>
                        ...
                        <div className="grid md:grid-cols-3 gap-8">
                            {[ 
                                ...
                                <motion.div key={i} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ delay: i * 0.15, duration: 0.8, ease: "easeOut" }}>
                                    <TiltCard className="card-glow p-8 rounded-xl h-full flex flex-col items-start transition-all duration-300 hover:shadow-2xl hover:shadow-blue-900/50 hover:border-blue-500/50">
                                        ...
                                        <h3 className="text-2xl font-bold mb-3 text-white">{core.title}</h3>
                                        <p className="text-gray-400 leading-relaxed text-base">{core.description}</p>
                                    </TiltCard>
                                </motion.div>
                            ))}
                        </div>
                ...
                {/*  Framework Section */}
                 <section id="ethics" className="py-20 px-4">
                    <div className="container mx-auto max-w-6xl">
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 0.8 }} className="text-center mb-20">
                            <h2 className="text-4xl md:text-5xl font-bold tracking-tight"> Framework</h2>
                        ...
                        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
                            {[ 
                                ...
                                { icon: Globe, title: 'Collective Flourishing', description: 'Helping you grow as an individual while also contributing positively to the wider community.' }
                            ].map((item, i) => (
                                <motion.div 
                                    key={i} 
                ...
                <section id="dilemmas" className="py-20 px-4">
                    <div className="container mx-auto max-w-3xl">
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 0.8 }} className="text-center mb-16">
                            <h2 className="text-4xl md:text-5xl font-bold tracking-tight">Explore Dilemmas</h2>
                            <p className="mt-4 text-gray-400 max-w-2xl mx-auto text-lg">Think through tough scenarios and see how N.E.H.A.'s core can power Thinking.</p>
                        </motion.div>
                        
                        <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 0.8, ease: "easeOut" }} className="space-y-8">
                            <div className="flex justify-center">
                                <button onClick={handleGenerateScenario} disabled={isGeneratingScenario} className="flex items-center gap-2 px-6 py-3 rounded-full font-bold text-blue-300 border-2 border-blue-500/50 hover:bg-blue-500/10 hover:border-blue-500 transition duration-300 disabled:border-gray-600 disabled:text-gray-500 disabled:cursor-not-allowed">
                                    {isGeneratingScenario ? <><div className="gemini-loader"></div><span>Generating...</span></> : <><span>✨ Figure out your Mind's processes</span></>}
                                </button>
                            </div>
                            
                            {scenario && (
                                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="text-center bg-gray-900/50 p-6 rounded-lg border border-gray-700 max-w-2xl mx-auto">
                                    <p className="font-serif text-xl italic text-gray-300">"{scenario}"</p>
                                </motion.div>
                            )}

                            {scenario && (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="w-full space-y-4 max-w-xl mx-auto">
                                    <textarea 
                                        value={userInput}
                                    ...
                                    />
                                    <button onClick={handleGetPerspective} disabled={isGettingPerspective || !userInput} className="w-full flex items-center justify-center gap-2 px-6 py-3 rounded-lg font-bold text-white bg-indigo-600 hover:bg-indigo-500 transition duration-300 disabled:bg-gray-600 disabled:cursor-not-allowed">
                                        {isGettingPerspective ? <><div className="gemini-loader"></div><span>Analyzing...</span></> : <><BrainCircuit className="w-5 h-5"/><span>Get NEHA's Perspective</span></>}
                                    </button>
                                </motion.div>
                            )}
                            ...
                            {perspective && (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl mx-auto">
                                     <h4 className="text-lg font-bold text-center mb-4 text-blue-300">NEHA's Analysis</h4>
                                    <div className="text-gray-400 whitespace-pre-wrap leading-relaxed bg-gray-900/50 p-6 rounded-lg border border-gray-700">{perspective}</div>
                                </motion.div>
                            )}
                        </motion.div>
                    </div>
                ...
                <section id="capabilities" className="py-20 px-4">
                    <div className="container mx-auto max-w-6xl">
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 0.8 }} className="text-center mb-20">
                            <h2 className="text-4xl md:text-5xl font-bold tracking-tight">Vespera's Integrated Ecosystem</h2>
                        ...
                        <div className="grid md:grid-cols-2 gap-x-12 gap-y-16">
                            {[ 
                                ...
                                { icon: Globe, title: 'The Human Operating System', description: 'Our long-term goal: a global system that helps improve health and share resources for the betterment of all humanity.' }
                            ].map((cap, i) => (
                                <motion.div key={i} initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 0.6, delay: i * 0.1 }} className="flex items-start space-x-6">
                    ...
                    <div className="container mx-auto max-w-4xl text-center">
                        <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, amount: 0.8 }} transition={{ duration: 0.8, ease: "easeOut" }}>
                            ...
                            <footer className="mt-8">
                                ...
                                <p className="text-gray-500">Chief Systems Philosopher, Vespera Project</p>
                            </footer>
                        </motion.div>
                    </div>
                </section>
                ...
                <section id="vision" className="relative py-32 sm:py-48 px-4 overflow-hidden">
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, amount: 0.8 }} transition={{ duration: 1.0, ease: [0.16, 1, 0.3, 1] }} className="container mx-auto max-w-5xl text-center relative z-20">
                        <h2 className="font-serif text-4xl md:text-6xl leading-tight text-transparent bg-clip-text bg-gradient-to-b from-gray-200 to-gray-500">
                            Our purpose is not to fashion another tool, but to bring forth a companion that guides you toward the stillness beyond the mind’s illusions.
                        </h2>
                    </motion.div>
                </section>
                  ...
                  <div className="container mx-auto max-w-4xl">
                    <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 0.8, ease: "easeOut" }}>
                        <TiltCard className="text-center card-glow p-10 md:p-16 rounded-xl border border-blue-500/20">
                          ...
                          <p className="text-lg text-gray-400 mb-8 max-w-2xl mx-auto">We're looking for partners in fields like AI, neuroscience, psychiatry and secure technology. If our vision resonates with you, we'd love to talk.</p>

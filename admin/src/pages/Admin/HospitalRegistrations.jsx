...
export const HospitalRegistrations = () => {
  const { hospitals, getAllHospitals, markHospitalAsReviewed, deleteHospital } = useContext(AdminContext);
  ...
  const handleMarkAsReviewed = async (id) => {
    await markHospitalAsReviewed(id);
  };

  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this hospital registration?')) {
  ...
  return (
    <div className="m-5 w-full">
      <p className="mb-3 text-lg font-medium">Hospital Registration Management</p>
          ...
          <div className="flex flex-col lg:flex-row items-start gap-10 text-gray-600">
            <div className="w-full lg:flex-1 flex flex-col gap-4">
              <p className="mb-3 text-lg font-medium flex items-center">
                <FiMail className="mr-2" /> Hospital Registration List
              </p>
              
              {hospitals.length === 0 ? (
                <p className="text-gray-500 text-center py-8">No hospital registration submissions yet.</p>
              ) : (
                <ul className="flex flex-col gap-2">
                  {hospitals.map((registration) => (
                    <li 
                    ...
                    >
                      ...
                      <p className="text-sm text-gray-600 mt-1 truncate">
                        {registration.type} | {registration.keyContact}
                      </p>
                    </li>
                  ))}
                </ul>
              ...
              {selectedRegistration ? (
                <div>
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h2 className="text-lg font-medium">{selectedRegistration.hospitalName}</h2>
                  ...
                  <div className="border rounded px-3 py-3 bg-gray-50">
                    ...
                    <p className="text-gray-700"><strong>Type:</strong> {selectedRegistration.type}</p>
                    <p className="text-gray-700"><strong>Year Established:</strong> {selectedRegistration.yearEstablished}</p>
                    <p className="text-gray-700"><strong>Key Contact:</strong> {selectedRegistration.keyContact}</p>
                    <p className="text-gray-700"><strong>Mental Health Professionals:</strong> {selectedRegistration.mentalHealthProfessionals}</p>
                    ...
                    {selectedRegistration.hospitalLogo && (
                      <p className="text-gray-700">
                        <strong>Logo:</strong> <a href={selectedRegistration.hospitalLogo} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">View Logo</a>
                      </p>
                    )}
                    {selectedRegistration.hospitalLicense && (
                      <p className="text-gray-700">
                        <strong>License Document:</strong> <a href={selectedRegistration.hospitalLicense} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">View License</a>
                      </p>
                    )}
                  </div>
